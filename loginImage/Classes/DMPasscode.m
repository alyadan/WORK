...
- (void)openPasscodeWithMode:(int)mode viewController:(UIViewController *)viewController {
    ...
    _passcodeViewController.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
...
/*   enteredCode - jufge pwsd
 *  _mode judge setting or matching
    _code judge first input code or second
...
//                UIAlertView* errorAlert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"dmpasscode_not_match", nil) message:nil
//            delegate:nil cancelButtonTitle:nil otherButtonTitles:NSLocalizedString(@"dmpasscode_okay", nil), nil];
//                [errorAlert show];
//                [self closeAndNotify:NO];
                [_passcodeViewController setErrorMessage:@"please input again!"];
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    [_passcodeViewController setInstructions:NSLocalizedString(@"dmpasscode_enter_new_code", nil)];
                    _count = 0;
                    [_passcodeViewController reset];
                    [_passcodeViewController setErrorMessage:nil];
                });
                return;
            }
        }
